  ...
  io.on("connection", (socket) => {
    ...
    socket.on("user:join", async (data) => {
      try {
        ...
        const room = await ChatRoom.findOne(query).populate(
          "user",
          "name email phone"
        );
        ...
        if (room) {
          ...
          console.log(
            "👋 New session, no room yet. Waiting for first message."
          );
    ...
    socket.on("admin:join", async () => {
      try {
        ...
        // ✅ FIX: Hiển thị FULL guestId thay vì cắt ngắn
        const formattedRooms = rooms.map((room) => ({
          ...room.toObject(),
          displayName:
            room.userType === "registered"
              ? room.user?.name || room.userName
              : room.guestId || "Khách", // ✅ FULL guestId, không cắt
        }));
      ...
      try {
        ...
        if (data.sender === "user") {
          ...
          if (!session.roomId) {
            ...
            if (isNew) {
              const roomWithUser = await ChatRoom.findById(room._id).populate(
                "user",
                "name email phone"
              );

              // ✅ FIX: Hiển thị FULL guestId
              const displayName =
                roomWithUser.userType === "registered"
                  ? roomWithUser.user?.name || roomWithUser.userName
                  : roomWithUser.guestId || "Khách"; // ✅ FULL guestId, không cắt
