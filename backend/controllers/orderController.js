...
module.exports = {
  ...
  createOrder: async (req, res) => {
    try {
      ...
      const { customer, payment_method, items, total, ward, district, city } =
        req.body;
      const fullAddress = `${customer.address}, ${ward || ""}, ${
        district || ""
      }, ${city}`;

      const today = new Date().toISOString().slice(2, 10).replace(/-/g, "");
      const random = Math.floor(1000 + Math.random() * 9000);
      const generatedOrderCode = `DH${today}${random}`;
      console.log("Guest checkout → Tạo order_code:", generatedOrderCode);
      ...
      const order = await OrderService.create({
        ...
        total,
        status:
          payment_method === "cod" || payment_method === "bank"
            ? "pending"
            : "paid",
        customer: {
          ...customer,
        ...
        city,
        order_code: `DH${new Date().toISOString().slice(2,10).replace(/-/g,'')}${Math.floor(1000 + Math.random()*9000)}`
      });
      ...
      if (customer.email) {
        try {
          ...
          console.error("Email lỗi (guest):", e);
        }
      }
  ...
  updateOrder: async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: error.message || "Lỗi cập nhật đơn hàng" });
    }
  },
      ...
      if (resultCode === 0) {
        ...
        try {
          ...
          if (updatedOrder) {
            ...
            console.log(
              "🛒 MoMo webhook - Clearing cart for user:",
            ...
            try {
              ...
              console.error(
                "❌ MoMo webhook - Error clearing cart:",
                cartError
              );
            ...
            try {
              ...
              const orderDetails = await OrderDetailService.getByOrderId(
                orderId
              );
              await EmailService.sendOrderStatusUpdate(
                updatedOrder,
              ...
              console.error(
                "❌ Lỗi gửi email thanh toán thành công:",
                emailError
              );
            }
  ...
  trackPublic: async (req, res) => {
    try {
      ...
      if (!order_code || !phone) {
        return res
          .status(400)
          .json({ message: "Vui lòng nhập mã đơn hàng và số điện thoại" });
      }
      ...
      const steps = [
        ...
        },
        {
          ...
          description: currentStep >= 3 ? "Đang xử lý" : "Chưa hoàn thành",
        },
        {
          title: "Đã bàn giao vận chuyển",
          completed: currentStep >= 4,
          description: currentStep >= 4 ? "Đang di chuyển" : "Chưa hoàn thành",
        },
        {
        ...
        },
        {
          ...
          description:
            currentStep >= 6 ? formatDate(order.updated_at) : "Chưa hoàn thành",
        },
      ];
